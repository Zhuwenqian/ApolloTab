...
# 内部模块导入
from .parser import GTPParser, parse_gtp, parse_score
...
class GTPPlayer:
    ...
    def __init__(self, gain: float = 0.7, sample_rate: int = 44100, buffer_size: int = 512):
        ...
        self._synth_engine: SynthEngine | None = None  # 音频合成器(延迟初始化)

        # ===== 数据状态 =====
        self._song: GTPSong | None = None  # 当前加载的歌曲对象
        ...
        # ===== 回调函数 =====
        self._note_callback: Callable | None = None  # 音符触发回调
    ...
    def init_audio(self, note_callback: Callable | None = None) -> bool:
        ...
        try:
            ...
            self._synth_engine = SynthEngine(
                gain=self._gain, sample_rate=self._sample_rate, buffer_size=self._buffer_size
            )
            ...
            if self._song:
                ...
                # 打印就绪信息
                mode_label = "全轨并轨" if self._audio_mode == self.MODE_ALL else "仅当前轨"
                duration_ms = self.get_current_duration_ms()
                print(
                    f"[GTPPlayer] ✓ 引擎就绪[{mode_label}]: "
    ...
    def get_current_duration_ms(self) -> float:
        ...
        if self._audio_mode == self.MODE_ALL:
            ...
            return self._midi_converter.get_total_duration_ms(self._song, self._current_track)
    ...
    def rebuild_audio_events(self) -> None:
        ...
        if len(self._expanded_measure_indices) != len(current_track.measures):
            print(
                f"[GTPPlayer] 反复记号展开: {len(current_track.measures)}小节 → "
        ...
        if self._audio_mode == self.MODE_ALL:
            # 全轨并轨: 转换所有音轨
            self._audio_events, self._track_channels = self._midi_converter.convert_all_tracks(
                self._song
            )
    ...
    def _sync_metronome(self) -> None:
        ...
        if self._song and self._song.tracks:
            ...
            if self._synth_engine.is_playing:
                ...
                start_offset_ticks = cur_ms / 1000.0 * bpm / 60.0 * ticks_per_beat
                # 还原为毫秒(传给 SynthEngine)
                start_offset_ms = start_offset_ticks * 60000.0 / (bpm * ticks_per_beat)
    ...
    def play_metronome_only(
        self, bpm: int = 120, numerator: int = 4, denominator: int = 4, duration_minutes: int = 10
    ) -> None:
        """
    ...
    def set_loop_region_by_measure(self, start_measure_idx: int, end_measure_idx: int) -> bool:
        """
        基于小节索引设置A/B区域循环
        ...
        print(
            f"[GTPPlayer] A/B循环设置: 小节[{start_measure_idx}-{end_measure_idx}] "
            f"= 时间[{loop_start_ms:.0f}ms - {loop_end_ms:.0f}ms] "
            f"(跨度{loop_end_ms - loop_start_ms:.0f}ms)"
        )
        return True

    def set_loop_region_by_position(
        self,
        start_pct: float,
        end_pct: float,
        ...
        print(
            f"[GTPPlayer] 循环查找: pct[{start_pct:.1f}-{end_pct:.1f}] "
    ...
    def _find_measure_at_time(self, time_ms: float) -> int | None:
        """
        根据时间戳查找所在的小节索引(内部方法)
        ...
        if not self._timeline_times or not self._playhead_timeline:
            print(
                f"[GTPPlayer] _find_measure_at_time: 时间线为空! "
        ...
        # [调试] 打印查找结果
        print(
            f"[GTPPlayer] _find_measure_at_time({time_ms:.0f}ms): "
    ...
    def find_measure_at_scroll_pos(self, scroll_y: float) -> dict | None:
        """
    ...
    def build_timeline(
        self, page_layouts: list, images: list[QPixmap], display_width: int
    ) -> list[dict]:
        """
            ...
            for sys_idx, system in enumerate(page.systems):
                ...
                for meas_idx, m_layout in enumerate(system.measures):
                    ...
                    for beat_idx, b_layout in enumerate(beats_in_measure):
                        ...
                        beats_before = (
                            sum(len(m2.beats) for m2 in system.measures[:meas_idx]) + beat_idx
                        )
        ...
        if self._expanded_measure_indices and len(self._expanded_measure_indices) > 0:
    ...
    def time_to_scroll_pos(
        self, time_ms: float, total_scroll_distance: float, display_height: int
    ) -> float:
        """
        根据音频时间计算对应的滚动Y位置
    ...
    def create_error_image(self, message: str, width: int = 800, height: int = 500) -> QPixmap:
        """
        ...
        painter.drawText(
            QRect(50, 40, width - 100, 60), Qt.AlignCenter, f"Guitar Pro 文件: {filename}"
        )
