...
def main() -> None:
    ...
    try:
        ...
        print("[错误] 无法导入 ApolloTab 库")
        print("请先安装: pip install ApolloTab")
        sys.exit(1)
    ...
    for i, page in enumerate(pages):
        # 构建输出文件名
        output_file = os.path.join(output_dir, f"{base_name}_t{track_index}_p{i + 1}.png")
    ...
    if renderer.last_layouts:
        ...
        total_measures = sum(
            len(sys.layouts) for page in renderer.last_layouts for sys in page.systems
        )
...
def batch_render(input_dir: str, output_dir: str = "./rendered") -> None:
    ...
    for file_name in files:
        ...
        try:
            ...
            for i, page in enumerate(pages):
                output_file = os.path.join(output_dir, f"{base_name}_p{i + 1}.png")
                page.save(output_file, "PNG")
